  ...
  const handleKeydown = React.useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      const setElement = () => {
        ...
        // Add the selected time to the array if it doesn't already exist
        setSelectedTimes((prevTimes) => {
          if (!prevTimes.includes(timeValue)) {
            const newTimes = [...prevTimes, timeValue];
            setdays(newTimes); // Update the global state with the new array
            return newTimes;
          }
          return prevTimes;
        });
